...
public class AddCakeServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 String tempPath = this.getServletContext().getRealPath("/cakeImage/temp");
		        	...
		        	if(item.isFormField()){
		               ...
		               System.out.println("上传的文件的扩展名是："+fileExtName);
		                 ...
		                 int len = 0;
		                   ...
		                   item.delete(); 
		                   message = "文件上传成功！";
		                   Cake cake=new Cake(0, value[0], imgPath, Float.parseFloat(value[1]), value[2], Float.parseFloat(value[3]), value[4], value[5], value[6], value[7], value[8]); 
		                   //System.out.println(cake);
		                   response.setContentType("text/html");
		           		   response.setCharacterEncoding("utf-8");
		           		   PrintWriter pwt=response.getWriter();
		                   if(cservice.addCakeInfo(cake)) {
		                	   pwt.println("<html><head></head><body><h1 style='color:green;'>商品新增成功，1秒后自动返回新增商品界面</h1></body></html>");
		           			   response.addHeader("Refresh", "1; url=TB/AddCake.jsp");
		                   }else {
		           			   pwt.println("<html><head></head><body><h1 style='color:red;'>商品新增失败，1秒后自动返回新增商品界面</h1></body></html>");
		           			   response.addHeader("Refresh", "1; url=TB/AddCake.jsp");
		        		}
		                  }
		                 }
