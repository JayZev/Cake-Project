						...
						<li>
							...
							<div id="subPages" class="collapse in">
								<ul class="nav">
									...
									<li><a href="RemoveCake.jsp" class="active">删除商品</a></li>
								</ul>
							</div>
						</li>
...
out.println("<tr><td style='vertical-align: middle'>蛋糕编号</td><td style='vertical-align: middle'>蛋糕名称</td>"+
			...
			"<td style='vertical-align: middle'>蛋糕状态</td><td style='vertical-align: middle'>操作</td></tr>");
											for (int i = 0; i < list.size(); i++) {
												//<button type='button' name='id' value='"+ck.getCid()+"'>"+ck.getCid()+"</button>  type='submit'
												Cake ck = list.get(i);
												out.println("<tr>");
												...
												out.println("<td style='vertical-align: middle'>"+ck.getC_state()+"</td>");
												out.println("<td style='vertical-align: middle'><button onClick='javascript:window.location.href=\"../RemoveCakeServlet?id="+ck.getC_id()+"\"' class='btn btn-danger'>删除</td>");
												out.print("</tr>");
											}
										%>
									</table>
									<p>
										<button type="button" class="btn btn-default" onclick="location.href='RemoveCake.jsp?pageid=<%=previd%>'">上一页</button>
											<%
												for (int i = 1; i < cake2.getPageNum()+1; i++) {
											%>
												
												<button type="button" class="btn btn-default" onclick="location.href='RemoveCake.jsp?pageid=<%=i %>'"><%=i %></button>
											<%
												}
											%>
								
										<button type="button" class="btn btn-default" onclick="location.href='RemoveCake.jsp?pageid=<%=nextid%>'">下一页</button>
										</p>
								</div>
							</div>
						</div>
